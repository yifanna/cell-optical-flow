...
from .tool import split_feature, merge_splits


def single_head_full_attention(q, k, v):
    # q, k, v: [B, L, C]#这里的L代表序列长度，C是通道数
...
class TransformerLayer(nn.Module):
    def __init__(self,
        ...
        if not self.no_ffn:
            ...
            #self.mlp2 = ConvolutionalGLU(512,hidden_features=512*2)
    ...
    def forward(self, source, target,
        ...
        if not self.no_ffn:
            ...
            #message = self.mlp2(torch.cat([source, message], dim=-1))
...
class FeatureFlowAttention(nn.Module):#框架图里的feature propagation
    """
