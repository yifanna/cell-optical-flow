...
#     # 提取角度 (色调 h)
#     angle = hsv_img[..., 0].astype(np.float32) * 2 * np.pi / 180  # 色调值范围是 0-180，需要转换为 0-360 度
#     # 提取幅度 (亮度 v)，并归一化到 [0,1] 范围
#     magnitude = hsv_img[..., 2].astype(np.float32) / 255.0  # 亮度的值范围是 0-255

#     # 使用角度和幅度计算光流的 u 和 v 分量
#     u = magnitude * np.cos(angle)  # 水平方向
#     v = magnitude * np.sin(angle)  # 垂直方向
...
#     average_migration_rate = np.mean(cell_magnitude)

#     return average_migration_rate

# # 读取光流图像
# optical_flow_img = cv2.imread('L:/fenge/true_flo/1/frame_0028_part_1.jpg')


# # 从颜色编码图像中提取 u 和 v 分量
# u, v = decode_optical_flow_color_image(optical_flow_img)

# # 读取或创建细胞区域掩码 (假设你已经手动标记了细胞区域)
# # cell_mask 应该是一个与光流图像相同尺寸的布尔数组，标记细胞区域
# cell_mask = cv2.imread('L:/fenge/true_dsc/1/frame_0028_part_1.jpg', cv2.IMREAD_GRAYSCALE)  # 读取细胞掩码图像
# cell_mask = cell_mask > 0  # 将掩码转换为布尔值
