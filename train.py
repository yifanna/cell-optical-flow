...
def train(args):
    ...
    if args.restore_ckpt is not None:
        # total_steps = int(args.restore_ckpt.split('_')[0])
        total_steps = int(args.restore_ckpt.split('/')[-1].split('_')[0])
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            ...
            if total_steps % VAL_FREQ == VAL_FREQ - 1:
                PATH = 'xycheckpoints/%d_%s.pth' % (total_steps+1, args.name)
            ...
            if total_steps > args.num_steps:
                ...
                break
            print(f"Iteration {total_steps}, Loss: {loss.item()}, EPE: {metrics['epe']}, 1px: {metrics['1px']}, 3px: {metrics['3px']}, 5px: {metrics['5px']}")
    logger.close()
    PATH = 'check/%s.pth' % args.name
    torch.save(model.state_dict(), PATH)
